  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setHours(data.time?.split(":")[0] || "0");
        setMinutes(data.time?.split(":")[1] || "0");
        setPortions(data.numberServings || 1);
        setCategory(data.category || "");
        setKitchen(data.kitchen || "");
        setCalories(data.calories || 0);
        setProteins(data.proteins || 0);
        setFats(data.fats || 0);
        setCarbs(data.carbs || 0);
        setRecipetip(data.recipeTip || "");
        if (data.recipeHistory) {
          const [title, desc] = data.recipeHistory.split("/", 2);
          setHistoryTitle(title || "");
          setHistoryDesc(desc || "");
          setShowHistory(true);
        }
        if (data.resultImage) {
          setPhoto(data.resultImage);
        }
        if (data.ingridients) {
          const parsedIngredients = data.ingridients
            .split(",")
            .map((item, index) => {
              const [textPart, countUnitPart] = item.split(":");
              if (!countUnitPart) {
                return {
                  uid: Date.now() + index,
                  text: textPart || "",
              ...
              const [countStr, ...unitParts] = countUnitPart.trim().split(" ");
    ...
    axios
      ...
      .then((res) => {
        ...
        const allsteps = data.map((item, index) => {
          const formula = item.stepFormula;
    ...
    if (!hasError) {
      ...
      try {
        const res = await axios.put(
          "http://localhost:5300/api/Recipe/updateRecipe/" + id,
          formData,
          {
            withCredentials: true,
          }
        ...
        if (res.status === 200) {
          try {
            await axios.delete(
              "http://localhost:5300/api/Images/deleteStepsByIdRecipe/" + id,
              {
                withCredentials: true,
              }
            );
            ...
            console.log("Ошибка в обновлении инструкции");
          }
        }
      } catch {
        console.log("Ошибка в обновлении рецепта");
      }
    }
  };
