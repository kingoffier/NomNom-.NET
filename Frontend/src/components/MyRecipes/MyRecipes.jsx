...
export default function MyRecipes() {
  ...
  useEffect(() => {
    if (user) {
      axios
        .get(
          "http://localhost:5300/api/Recipe/getAllRecipeByIdUser/" + user.userId
        )
        .then((res) => {
          setCount(res.data);
        })
  ...
  useEffect(() => {
    if (count) {
      for (let index = 0; index < count.length; index++) {
        axios
          .get(
            "http://localhost:5300/api/Recipe/getRecipeById/" + count[index]?.id
          )
          .then((res) => {
            setRecipes((prev) => {
  ...
  function editRecipePage(idRecipe) {
    navigate("/editRecipe/"+idRecipe);
  }
  function deleteRecipeModal() {
    if (idRecipe != 0) {
      axios
        .delete("http://localhost:5300/api/Recipe/deleteRecipe/" + idRecipe, {
          withCredentials: true,
        })
        .then((res) => {
    ...
    <div>
      ...
      <main>
        <div>
          <div className={style.page}>
            <div className={style.centerpage}>
              <div className={style.title}>
                <h1 className={style.firsth1}>Мои рецепты</h1>
                <p className={style.titlep}>То, что сделал сам</p>
              </div>
                          ...
                          <div className={style.bot}>
                            <button
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                editRecipePage(recipe.id);
                              }}
                            >
                              <div className={style.editrecipe}>
                            ...
                            >
                              <div className={style.removerecipe}>
          ...
          {isModal ? (
            <div className={style.modalBackdrop1} id="modalbackDrop1">
              <div className={style.modal1}>
                <h2>Вы уверены что хотите удалить рецепт?</h2>
                <div className={style.choicemodal}>
                  ...
                  <button
                    ...
                    data-recipe-id=""
                    onClick={deleteRecipeModal}
                    className={style.modalyes}
                  >
                    Да
                  </button>
                </div>
